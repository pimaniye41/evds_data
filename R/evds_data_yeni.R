...
evds_data_2 <- function(anahtar,
  ...
  response <- httr::GET(
    url = veriadresi,
    httr::add_headers("key" = anahtar)
  )

  csvveri <- httr::content(response, "text")
  veridf <- readr::read_csv(csvveri)
  veridf <- veridf %>% dplyr::select(-UNIXTIME)

  if ("YEARWEEK" %in% colnames(veridf)) {
    veridf <- veridf %>% dplyr::select(-YEARWEEK)
  }

  veridf$Tarih <- if (nchar(veridf$Tarih[1]) == 10) {
    as.Date.character(veridf$Tarih, format = "%d-%m-%Y")
  } else {
    veridf$Tarih <- stringr::str_replace_all(string = veridf$Tarih, "Q1", "03-01")
    veridf$Tarih <- stringr::str_replace_all(string = veridf$Tarih, "Q2", "06-01")
    veridf$Tarih <- stringr::str_replace_all(string = veridf$Tarih, "Q3", "09-01")
    veridf$Tarih <- stringr::str_replace_all(string = veridf$Tarih, "Q4", "12-01")
    veridf$Tarih <- stringr::str_replace_all(string = veridf$Tarih, "S1", "06-01")
    veridf$Tarih <- stringr::str_replace_all(string = veridf$Tarih, "S2", "12-01")

    if (nchar(veridf$Tarih[1]) == 6 | nchar(veridf$Tarih[1]) == 7) {
      veridf$Tarih <- paste(veridf$Tarih, "-01", sep = "")
      as.Date.character(veridf$Tarih, format = "%Y-%m-%d")
    } else {
  ...
  veridf <- veridf %>% dplyr::mutate(across(-1, as.numeric))
  return(veridf)
}
